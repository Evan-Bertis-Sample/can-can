...
using UnityEngine;
using System.Linq;
namespace FormulaBoy.DSA.StateMachine
{
    /// <summary>
    ...
    public sealed class StateMachine<StateContextType> : IState<StateContextType>
    {
        ...
        /// <summary>
        /// Returns a state with the given type, if it exists. Otherwise, returns null.
        /// </summary>
        /// <typeparam name="T"> The type of state </typeparam>
        public T GetState<T>() where T : class, IState<StateContextType>
        {
            return ManagedStates.OfType<T>().FirstOrDefault();
        }

        /// <summary>
        /// Returns the lambda state with the given name, if it exists. Otherwise, returns null.
        /// </summary>
        /// <param name="name"> The name of the lambda state</param>
        public LambdaState<StateContextType> GetLambdaState(string name)
        {
            return ManagedStates.OfType<LambdaState<StateContextType>>().FirstOrDefault(state => state.Name == name);
        }
    }
}
